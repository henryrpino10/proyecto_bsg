...
class DataTransformer:
    ...
    def prepare_for_hive(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Convertir timestamps a string ANTES de renombrar
        for col in df_hive.select_dtypes(include=["datetime64"]).columns:
            df_hive[col] = df_hive[col].astype(str)

        # Renombrar columnas que son palabras reservadas en Hive
        rename_map = {
        ...
        timestamp_col = "detection_timestamp" if "detection_timestamp" in df_hive.columns else "timestamp" if "timestamp" in df_hive.columns else None
        if timestamp_col:
            try:
                df_hive = df_hive.sort_values(timestamp_col)
            except:
                pass
    ...
    def rename_for_hive(self, df: pd.DataFrame) -> pd.DataFrame:
        """Renombra columnas que son palabras reservadas en Hive."""
        rename_map = {
            'timestamp': 'detection_timestamp',
            'filename': 'source_filename',
            '_extracted_at': 'extracted_at',
